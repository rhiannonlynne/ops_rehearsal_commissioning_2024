...
seeing_std_g = [photrep_dict_alltracts[key]['g']['seeing']['std'] for key in photrep_dict_alltracts.keys()]
seeing_ct_g = [photrep_dict_alltracts[key]['g']['seeing']['count'] for key in photrep_dict_alltracts.keys()]
...
airmass_std_g = [photrep_dict_alltracts[key]['g']['airmass']['std'] for key in photrep_dict_alltracts.keys()]
airmass_ct_g = [photrep_dict_alltracts[key]['g']['airmass']['count'] for key in photrep_dict_alltracts.keys()]
...
for i in range(len(seeing_std_g)):
    seeing_stats = seeing_std_g[i]
    seeing_counts = seeing_ct_g[i]
...
plt.title('g-band; S/N>50, nvisits>30')
plt.minorticks_on()
plt.xlim(0.38, 2.18)
plt.ylim(2, 38)
plt.legend(ncol=4)

plt.savefig('auxtel_photrep_seeing_gband_alltracts.png')
plt.close()
...
plt.savefig('auxtel_photrep_seeing_rband_alltracts.png')
plt.close()
...
plt.savefig('auxtel_photrep_seeing_iband_alltracts.png')
plt.close()

for i in range(len(airmass_std_g)):
    airmass_stats = airmass_std_g[i]
    airmass_counts = airmass_ct_g[i]
...
plt.savefig('auxtel_photrep_airmass_gband_alltracts.png')
plt.close()
...
plt.savefig('auxtel_photrep_airmass_rband_alltracts.png')
plt.close()
...
plt.savefig('auxtel_photrep_airmass_iband_alltracts.png')
plt.close()
